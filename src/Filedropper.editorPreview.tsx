import { ReactElement, createElement } from "react";
// import { HelloWorldSample } from "./components/HelloWorldSample";
import { FiledropperPreviewProps } from "../typings/FiledropperProps";

const defaultUploadImage = "data:image/png;base64,";

export function preview({ buttonText, defaultText, acceptedFilesText, acceptedFileTypes, maxFileSize, uploadIcon, uploadImage, uploadImageType }: FiledropperPreviewProps): ReactElement {
    return (
        <div className="dropzone">
                {renderUploadImage(uploadIcon, uploadImage, uploadImageType)}
            <p>{defaultText}</p>
            <button type="button" className="fileSelectButton">
                {buttonText}
            </button>
            {acceptedFilesString(acceptedFilesText, acceptedFileTypes, maxFileSize)}
        </div>
    )
}

const acceptedFilesString = (acceptedFilesText: string, acceptedFileTypes: string, maxFileSize: number | null) =>{
    if (acceptedFilesText)
        return <p className="acceptedfiles">{acceptedFilesText} Max file size: {maxFileSize} Mbs</p>;
    else {
        if (acceptedFileTypes)
            return <p className="acceptedfiles">Accepted file types: {acceptedFileTypes}. Max file size: {maxFileSize} Mbs</p>;
        else
            return <p className="acceptedfiles">Max file size: {maxFileSize} Mbs</p>;;
    }
}

const renderUploadImage = (uploadIcon: { type: "glyph"; iconClass: string; } | { type: "image"; imageUrl: string; iconUrl: string; } | { type: "icon"; iconClass: string; } | undefined, uploadImage: { type: "static"; imageUrl: string; } | { type: "dynamic"; entity: string; } | null | undefined, uploadImageType: string | undefined) => {
    if (uploadIcon === undefined && uploadImage === undefined) {
        return <img className="uploadImage" src={defaultUploadImage} />
    }
    else {
        if (uploadImageType === "image") {
            //@ts-ignore
            return <img className="uploadImage" src={uploadImage?.value?.uri} />
        }
        else {
            //@ts-ignore
            return <span className={"uploadIcon glyphicon " + uploadIcon?.value?.iconClass} />
        }
    }
}

export function getPreviewCss(): string {
    return require("./ui/Filedropper.css");
}
